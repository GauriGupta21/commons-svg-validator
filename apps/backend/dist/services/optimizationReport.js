...
function generateOptimizationReport(content) {
    const pathCount = (content.match(/<path/gi) || [])
        .length;
    const groupCount = (content.match(/<g/gi) || [])
        .length;
    const imageCount = (content.match(/<image/gi) || [])
        .length;
    const textCount = (content.match(/<text/gi) || [])
        .length;
    const defsCount = (content.match(/<defs/gi) || [])
        .length;
    const metadataPresent = /<metadata/i.test(content);
    ...
    if (estimatedOptimizationPercent > 100) {
