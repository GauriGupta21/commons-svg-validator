...
import type { RepairReport } from "../types/suggestion";
...
function removeEditorMetadata(svg: string, fixes: string[]): string {
  ...
  if (next !== svg) fixes.push("Removed editor metadata (Inkscape/Sodipodi/Adobe)");
  return next;
}

function removeExternalReferences(svg: string, fixes: string[]): string {
  let next = svg.replace(/\s+xlink:href="(?!#)[^"]*"/gi, "");
  next = next.replace(/(<(?!image)[a-z]+[^>]*)\s+href="(?!#)[^"]*"/gi, "$1");
  if (next !== svg) fixes.push("Removed external URL references");
  return next;
}

function removeDangerousHandlers(svg: string, fixes: string[]): string {
  return applyFix(svg, /\s+on[a-z]+="[^"]*"/gi, "", "Removed inline event handlers", fixes);
}

function addViewBox(svg: string, fixes: string[]): string {
  ...
  if (next !== svg) fixes.push(`Added viewBox (0 0 ${w} ${h})`);
  return next;
}

function removeEmptyGroups(svg: string, fixes: string[]): string {
  let current = svg;
  for (let i = 0; i < 10; i++) {
  ...
  if (current !== svg) fixes.push("Removed empty groups");
  return current;
}

function removeComments(svg: string, fixes: string[]): string {
  return applyFix(svg, /<!--[\s\S]*?-->/g, "", "Removed comments", fixes);
}

function minify(svg: string, fixes: string[]): string {
  ...
  try {
    ...
    if (result.data !== svg) fixes.push("Minified SVG");
    return result.data;
  } catch {
...
export function repairSVG(content: string): RepairResult {
  const fixes: string[] = [];
